...
### M1 — content pass A (levels 1–4) — DONE (first drafts)
- The ladder's four maps are generated by `tools/build_level.lua` (the
  level DSL) and previewed with `tools/render_map.py`; they live in
  `config.levels` (the workshop maps are hidden from the select).
  - meadow: walk/jump, keys → locks → doors, exit; no enemies
  - battlements: archers, the first switch-shot gate, sticky bounce
  - the crossing: one rope swing over a void, melee chasers, checkpoints
  - springside: spring vaults, a phase wall, checkpoints, an exit archer
- Checkpoint entities are engine-backed (kind `checkpoint`).
- Each level's swing/flow is verified headlessly (`levels_flow_test`).
- Remaining in M1: a human playtest pass for pacing, par/gold retuning.

### M2 — content pass B (levels 5–8) — DONE (first drafts)
- The ladder's back half is generated by `tools/build_level.lua`:
  - arrowslit: lasers + arrow slits (slits block the laser's *sight*
    but pass your arrows — the counterplay; verified headlessly)
  - winchyard: three void crossings zipped by rope arrows into winches
    (capture -> reel -> throw verified on the real maps)
  - the vault: two key/door groups; a floating key only a bomb ride
    reaches; a laser, an archer perch and melee guards
  - the keep: rocketeers, a bomber trench, a laser/archer gate, melee
    crowd — the full gauntlet
- The whole ladder plays against a plain sky blue: no level carries
  backdrop sprites (the Background layer is parsed but never drawn).
- Remaining in M2: human playtest of 5–8, par/gold retuning, enemy
  count/difficulty tuning per level.
- Remaining in M2: human playtest of 5–8, par/gold retuning, enemy
  count/difficulty tuning per level.
- Per-instance enemy tuning (object properties in Tiled) beats global
