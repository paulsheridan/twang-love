...
--   3. jump/aim/swap start the highlighted level: a fresh load of that
--      map (new world/ents, full-health player) and the sim resumes
--   4. m / start don't open the test menu in select mode
--
...
do
  ...
  nudge("down")
  assert_true(g.level_sel == 2, "down moves the cursor")
  for _ = 1, n - 1 do nudge("down") end  -- to the last row and wrap around
  assert_true(g.level_sel == 1, "down wraps from the last row to the first")
  nudge("up")
  assert_true(g.level_sel == n, "up wraps from the first row to the last")
  nudge("up")
  assert_true(g.level_sel == n - 1, "up moves the cursor")
end
...
do
  ...
  g.mode = "select"
  step(env)
  keys.down = true; step(env); keys.down = nil; step(env)  -- row 2
  assert_true(g.level_sel == 2, "row 2 selected")
